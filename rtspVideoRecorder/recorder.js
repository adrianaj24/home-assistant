...
const FileHandler = require("../../rtspVideoRecorder/fileHandler.js/index.js");
...
const RTSPRecorder = class {
  ...
  getFilename(folderPath) {
    return path.join(
      folderPath,
      moment().format(this.fileNameFormat) + this.getExtenstion()
    );
  }
  ...
  recordStream() {
    ...
    const folderPath = this.getMediaTypePath();
    fh.createDirIfNotExists(folderPath);
    const fileirName = this.getFilename(folderPath);
    this.writeStream = this.getChildProcess(fileName);
